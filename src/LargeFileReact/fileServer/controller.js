...
// 大文件存储目录 demo directory
const UPLOAD_DIR = path.resolve(__dirname, '..', 'target');

// 提取后缀名 get file extension
const extractExt = (filename) =>
  filename.slice(filename.lastIndexOf('.'), filename.length);

// 写入文件流 write to file stream
...
// create a directory for temporary storage of chunks. add the 'chunkDir' prefix to distinguish it from the chunk name
const getChunkDir = (fileHash) =>
  path.resolve(UPLOAD_DIR, `chunkDir_${fileHash}`);

// 返回已经上传的所有切片名
const createUploadedList = async (fileHash) => {
  // readdir方法 遍历指定文件夹下所有的文件夹
...
module.exports = class {
  ...
  async deleteFiles(req, res) {
    ...
    res.end(JSON.stringify({ code: 0, message: 'delete files success' }));
  }
