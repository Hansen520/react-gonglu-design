...
import { Base, ConfigData, defaultState } from './utils/type';
...
const ScrollBoard = forwardRef(
  ({ onClick, config, className, style, onMouseOver }: Base, ref: any) => {
    ...
    const handleData = (config: ConfigData): ConfigData => {
      ...
      config.dataSource.forEach((rows) => {
        ...
        config.columns.forEach((cols) => {
          /* 在原来的基础上进行回调 */
          const val = cols.render
            ? cols.render(rows[cols.dataIndex], rows)
            : formatTip(rows[cols.dataIndex], cols);
          inData.push(val);
        });
        data.push(inData);
      });
    ...
    const calcData = () => {
      /* 合并配置项 */
      const mergedConfig: any = handleData(
                  ...
                  >
                    {row.ceils.map((ceil: any, ci: any) => (
                      <div
                        ...
                        onMouseEnter={() =>
                          handleHover(true, ri, ci, row, ceil)
                        }
